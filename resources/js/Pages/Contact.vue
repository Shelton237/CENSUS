    ...
    <MainLayout>
        ...
        <section class="relative pt-32 pb-20 overflow-hidden bg-[#204138]">
            ...
            <div class="container relative z-10 text-center">
                <span class="inline-block py-1 px-3 rounded-full bg-[#EDAF11]/20 text-[#EDAF11] font-bold text-sm tracking-widest uppercase mb-4">{{ __('Contactez-nous') }}</span>
                <h1 class="text-4xl md:text-5xl font-black text-white mb-6 leading-tight">{{ __('Nous contacter') }}</h1>
                <p class="text-xl text-white/80 max-w-2xl mx-auto leading-relaxed">{{ __('Une question sur le recensement ou une requête administrative ? Notre équipe vous répondra dans les meilleurs délais.') }}</p>
            </div>
        </section>
                ...
                <div style="max-width: 650px; margin: 0 auto; background: white; padding: 50px; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.06); border: 1px solid #F0F0F0;">
                    ...
                    <form @submit.prevent="submit">
                        <div style="margin-bottom: 25px;">
                            <label style="display: block; font-weight: 700; margin-bottom: 10px; font-size: 14px; text-transform: uppercase; color: #555;">{{ __('Nom complet') }}</label>
                            <input v-model="form.name" type="text" style="width: 100%; padding: 16px; border: 2px solid #F0F0F0; border-radius: 12px; font-size: 16px; outline: none; transition: border-color 0.2s;" :placeholder="__('Jean Dupont')" required>
                        </div>

                        <div style="margin-bottom: 25px;">
                            <label style="display: block; font-weight: 700; margin-bottom: 10px; font-size: 14px; text-transform: uppercase; color: #555;">{{ __('Adresse E-mail') }}</label>
                            <input v-model="form.email" type="email" style="width: 100%; padding: 16px; border: 2px solid #F0F0F0; border-radius: 12px; font-size: 16px; outline: none; transition: border-color 0.2s;" :placeholder="__('jean.dupont@exemple.cm')" required>
                        </div>

                        <div style="margin-bottom: 35px;">
                            <label style="display: block; font-weight: 700; margin-bottom: 10px; font-size: 14px; text-transform: uppercase; color: #555;">{{ __('Votre Message') }}</label>
                            <textarea v-model="form.message" style="width: 100%; height: 160px; padding: 16px; border: 2px solid #F0F0F0; border-radius: 12px; font-size: 16px; outline: none; resize: vertical; transition: border-color 0.2s;" :placeholder="__('Comment pouvons-nous vous aider ?')" required></textarea>
                        </div>

                        <button type="submit" :disabled="form.processing" style="background: #295E4D; color: white; border: none; padding: 18px 35px; font-size: 16px; font-weight: 900; border-radius: 12px; cursor: pointer; width: 100%; transition: transform 0.2s, background 0.2s;">
                            {{ __('ENVOYER LE MESSAGE') }}
                        </button>
                    </form>

                    <!-- Coordonnées de contact -->
                    <div style="margin-top: 50px; border-top: 1px solid #EEE; padding-top: 36px;">
                        <p style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.14em; color: #9CA3AF; margin-bottom: 20px; text-align: center;">{{ __('Autres moyens de nous joindre') }}</p>
                        <div style="display: flex; flex-direction: column; gap: 12px;">
                            ...
                            <a href="tel:+237693873145" style="display: flex; align-items: center; gap: 14px; padding: 16px 18px; background: #F7F9F8; border-left: 4px solid #204138; border-radius: 12px; text-decoration: none; transition: background 0.2s;">
                                ...
                                <div>
                                    <p style="font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.12em; color: #9CA3AF; margin: 0 0 2px;">{{ __('Téléphone') }}</p>
                                    <p style="font-size: 17px; font-weight: 800; color: #204138; margin: 0; letter-spacing: 0.04em;">(+237) 6 93 87 31 45</p>
                                </div>
                            </a>
                            ...
                            <a href="mailto:recensement90@gmail.com" style="display: flex; align-items: center; gap: 14px; padding: 16px 18px; background: #F7F9F8; border-left: 4px solid #EDAF11; border-radius: 12px; text-decoration: none; transition: background 0.2s;">
                                ...
                                <div>
                                    <p style="font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.12em; color: #9CA3AF; margin: 0 0 2px;">{{ __('Adresse e-mail') }}</p>
                                    <p style="font-size: 15px; font-weight: 700; color: #204138; margin: 0;">recensement90@gmail.com</p>
                                </div>
                            </a>
                            ...
                            <a href="https://m.me/Cameroon-CENSUS-2026" target="_blank" rel="noopener noreferrer" style="display: flex; align-items: center; gap: 14px; padding: 16px 18px; background: #F7F9F8; border-left: 4px solid #1877F2; border-radius: 12px; text-decoration: none; transition: background 0.2s;">
                                ...
                                <div>
                                    <p style="font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.12em; color: #9CA3AF; margin: 0 0 2px;">{{ __('Messenger') }}</p>
                                    <p style="font-size: 15px; font-weight: 700; color: #204138; margin: 0;">Cameroon-CENSUS-2026</p>
                                </div>
                            </a>
