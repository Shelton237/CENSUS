...
const deleteArticle = (slug) => {
    if (confirm('Êtes-vous sûr de vouloir supprimer cet article ?')) {
        form.delete(route('admin.articles.destroy', slug));
    }
};
</script>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="article in articles" :key="article.id" class="hover:bg-[#f9fbfb] transition-colors">
                                ...
                                <td class="px-6 py-4 text-right flex justify-end gap-3">
                                    <Link :href="route('admin.articles.edit', article.slug)" class="text-gray-400 hover:text-[#EDAF11] transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </Link>
                                    <button @click="deleteArticle(article.slug)" class="text-gray-400 hover:text-red-500 transition-colors">
